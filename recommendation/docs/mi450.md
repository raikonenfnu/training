...
generic register exhaustion. ROCgdb's debug agent stopped all four waves at
the first final-DV `global_store_b128`. Its address pair was correct in lanes
1--31, while lane 0 was displaced by exactly `0x6d0c6e1a` in every wave. The
producer multiplies the long-lived V-lane offset by two, so the corrupt input
is `0x3686370d` -- plausible FP32 accumulator data -- instead of lane 0's
integer zero. Later address fragments derived from the same offset before the
WMMA region remain correct. The bad value is therefore a live VGPR overwrite,
not bad `seq_offsets`, pointer arithmetic, or an asynchronous report from a
...
The original binary with the same seed faults. Disabling only
`amdgpu-sched-strategy=coexec` also survives 7000 iterations on LLVM `b010a18d`
and 4000 on `ce352942`; disabling expert mode while retaining the coexec
attribute still faults. Thus coexec's machine schedule is causal, while high
VGPR count alone is not.
...
backend defect is that the gfx1250-A0 coexec schedule is still too aggressive
for this WMMA/high-VGPR-mode combination; the architectural symptom is an
