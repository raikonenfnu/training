...
gfx1250 limit of 1024 VGPRs and spills. With the coexec scheduler disabled the
944-VGPR reduction survives 7000 iterations, but the full 1024-VGPR kernel
still faults with either scheduler. The fault is a write (`RW=1`) to a varying,
far-away mapped address. Taken together with the source-level bounds audit and
intact output guards, this points to address-register corruption in the AMD
backend at extreme register pressure, rather than a reproducible source-level
off-by-one. That compiler-level mechanism remains an inference: the runtime
does not report a faulting PC.
