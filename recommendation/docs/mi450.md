...
LLIR attribute is the trigger, but the failing transformation is in LLVM's
LLIR-to-AMDGPU machine scheduling/register-allocation path (or an A0 hardware
hazard that this path must model). The `global_store_b128` is only where the
already-corrupted address becomes observable; it is not itself mis-lowered.
