...
zero scratch. A machine-code override that fully separates each WMMA with
`s_nop 7` survives 4000 iterations. One-cycle `s_nop 0` padding is not enough:
it produced a misleading 1000-iteration pass and then faulted at step 0 in the
4000-iteration repeat. Padding either half of the WMMA sites alone and padding
only the v258:v265-adjacent WMMA also fault. This rules out one accidental
overlap and shows a repeated WMMA co-execution timing violation. The remaining
backend defect is broader than coexec alone: gfx1250-A0 machine
scheduling/register allocation does not protect this long-lived extended VGPR
from the WMMA co-execution hazard in every schedule mode. Coexec makes the
hazard much easier to hit, while the full 1024-VGPR allocation can expose the
same overwrite with coexec off. The architectural symptom is an
accumulator-like write into live v257. Buffer addressing is not on this path
(`AMDGCN_USE_BUFFER_OPS=0`) and is a separate issue.
